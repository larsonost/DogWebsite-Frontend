...
//Chen
export const userFollowsAnotherUser = async (followed, action) => {
  const endpoint = action === "follow" ? "follow" : "unfollow";
  const response = await api.post(`${USERS_URL}/${followed}/${endpoint}`);
  return response.data;
};

// export const userUnfollowsAnotherUser = async (followed) => {
//   const response = await api.delete(`${SERVER_API_URL}/unfollow/${followed}`);
...
export const getFollowerUsers = async (userId) => {
